...
async function renderHistory(){
  ...
  try{const {data}=await listMyActivities();activityRows=data||[];if($('printActivityRecordsBtn'))$('printActivityRecordsBtn').disabled=!activityRows.length;if(!data.length){box.innerHTML='<div class="records-empty">Sin registro</div>';return;}box.innerHTML=`<div class="records-scroll"><table class="records-table"><thead><tr><th>Fecha</th><th>Deporte</th><th>Minutos</th><th>Intensidad</th><th>Distancia</th><th>Calorías</th></tr></thead><tbody>${data.map(a=>`<tr><td>${a.activity_date}</td><td>${a.sport}</td><td>${a.minutes} min</td><td>${a.intensity}</td><td>${a.distance??'—'}</td><td>${a.calories} kcal</td></tr>`).join('')}</tbody></table></div>`;}catch(e){toast(`❌ ${e.message}`,'error');}finally{box.removeAttribute('aria-busy');}
}
async function renderRankings(){
  if(!context.session)return;try{const s=await getSupabase(),call=async sport=>{const {data,error}=await s.rpc('live_weekly_rankings',{p_sport:sport});if(error)throw error;return (data||[]).map(r=>({...r,position:r.rank_position}));},general=await call(null),bySport=Object.fromEntries(await Promise.all(rankingSports.map(async sport=>[sport,await call(sport)])));const avatar=r=>r?.avatar_url?`<img class="rank-avatar" src="${escape(r.avatar_url)}" alt="Foto de ${escape(r.full_name)}">`:`<div class="rank-avatar avatar-fallback">${escape(r?.full_name?.[0]||'–')}</div>`,empty=i=>({position:i,full_name:'Sin registro',total_minutes:0,dominant_sport:'Sin registro'}),top=[general[0]||empty(1),general[1]||empty(2),general[2]||empty(3)];
...
async function renderProfile(){
  if(!context.session)return;const s=await getSupabase();const [d,posts,articles,goal,pref]=await Promise.all([progressSnapshot(),s.from('community_posts').select('id,description,sport,created_at').eq('user_id',context.profile.id).eq('visibility','private').order('created_at',{ascending:false}).limit(20),s.from('saved_articles').select('id,title,url,sport,source,created_at').eq('user_id',context.profile.id).order('created_at',{ascending:false}),s.from('goals').select('*').eq('user_id',context.profile.id).eq('status','active').order('created_at',{ascending:false}).limit(1).maybeSingle(),s.from('whatsapp_preferences').select('*').eq('user_id',context.profile.id).maybeSingle()]);
  ...
  if(pref.data){if($('whatsappPhone'))$('whatsappPhone').value=pref.data.phone_e164||'';if($('whatsappEnabled'))$('whatsappEnabled').checked=!!pref.data.enabled;}
...
function printDocument(title,content){const popup=window.open('','_blank','noopener,noreferrer');if(!popup)return toast('Permite ventanas emergentes para imprimir.','warning');popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${escape(title)}</title><style>body{font:14px Arial;color:#092d6d;padding:28px}h1{color:#08285f}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd6e7;padding:8px;text-align:left}th{background:#0b3478;color:white}.meta{margin-bottom:20px}@media print{button{display:none}}</style></head><body><h1>${escape(title)}</h1><p class="meta">Usuario: ${escape(context.profile?.full_name||'')} · ${new Date().toLocaleDateString('es-MX')}</p>${content}<script>window.onload=()=>window.print()<\/script></body></html>`);popup.document.close();}
function printActivities(){if(!activityRows.length)return;printDocument('Registro de actividades',`<table><thead><tr><th>Fecha</th><th>Deporte</th><th>Minutos</th><th>Intensidad</th><th>Distancia</th><th>Calorías</th><th>Notas</th></tr></thead><tbody>${activityRows.map(a=>`<tr><td>${escape(a.activity_date)}</td><td>${escape(a.sport)}</td><td>${a.minutes}</td><td>${escape(a.intensity)}</td><td>${escape(a.distance??'—')}</td><td>${a.calories}</td><td>${escape(a.notes||'')}</td></tr>`).join('')}</tbody></table>`);}
...
document.querySelectorAll('[data-sport]').forEach(b=>b.addEventListener('click',()=>{productionSport=b.dataset.sport;sessionStorage.setItem('sumandoMinutosUltimaSeccion','deportes');sessionStorage.setItem('sumandoMinutosUltimoDeporte',productionSport);setTimeout(()=>Promise.all([renderHistory(),renderSportCalories(),renderCloudEvidence(productionSport)]).catch(e=>toast(`❌ ${e.message}`,'error')),0);},true));

...
async function onRegister(event){
  event.preventDefault(); event.stopImmediatePropagation(); const button=event.currentTarget.querySelector('button[type=submit]'); busy(button,true,'Creando cuenta…');
  try{
    ...
    $('registerModal').classList.remove('open'); event.currentTarget.reset(); toast('✅ Cuenta creada. Revisa tu correo para confirmarla.','success');
  }catch(error){toast(`❌ ${error.message}`,'error');}finally{busy(button,false);}
}
...
async function onPost(event){event.preventDefault();event.stopImmediatePropagation();try{await createPost({description:$('postDescription').value.trim(),sport:$('postSport').value,visibility:event.currentTarget.elements.postVisibility.value,image:$('postImageInput').files[0]});event.currentTarget.reset();event.currentTarget.classList.remove('open');toast('✅ Publicación guardada','success');await renderPosts();}catch(e){toast(`❌ ${e.message}`,'error');}}
...
$('logoutBtn')?.addEventListener('click',async e=>{e.preventDefault();e.stopImmediatePropagation();try{await signOut();context={session:null,profile:null};applyContext();toast('Sesión cerrada correctamente');}catch(error){toast(`❌ ${error.message}`,'error');}},true);
...
document.querySelectorAll('[data-sport]').forEach(b=>b.addEventListener('click',()=>{productionSport=b.dataset.sport;setTimeout(renderHistory,0);},true));
